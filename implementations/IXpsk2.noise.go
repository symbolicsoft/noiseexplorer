...
func writeMessageB(hs handshakestate, payload []byte) (handshakestate, messagebuffer, cipherstate, cipherstate) {
	ss, s, e, rs, re, psk, initiator := hs.ss, hs.s, hs.e, hs.rs, hs.re, hs.psk, hs.i
	ne, ns, ciphertext := emptyKey, []byte{}, []byte{}
	e = generateKeypair()
	ne = e.pk
...
func readMessageB(hs handshakestate, message messagebuffer) (handshakestate, []byte, bool, cipherstate, cipherstate) {
	ss, s, e, rs, re, psk, initiator := hs.ss, hs.s, hs.e, hs.rs, hs.re, hs.psk, hs.i
	valid1 := true
	re = message.ne
	ss = mixHash(ss, re[:])
...
func SendMessage(session noisesession, message []byte) (noisesession, messagebuffer) {
	...
	if session.mc == 1 {
		hs, messageBuffer, session.cs1, session.cs2 = writeMessageB(hs, message)
	}
	if session.mc > 1 {
		if hs.i {
...
func RecvMessage(session noisesession, message messagebuffer) (noisesession, []byte, bool) {
	...
	if session.mc == 1 {
		hs, plaintext, valid, session.cs1, session.cs2 = readMessageB(hs, message)
	}
	if session.mc > 1 {
		if hs.i {
			hs.ss.cs = session.cs2
