...
func writeMessageA(hs handshakestate, payload []byte) (handshakestate, messagebuffer, cipherstate, cipherstate) {
...
func readMessageA(hs handshakestate, message messagebuffer) (handshakestate, []byte, bool, cipherstate, cipherstate) {
	ss, s, e, rs, re, psk, initiator := hs.ss, hs.s, hs.e, hs.rs, hs.re, hs.psk, hs.i
	valid1 := true
	ss = mixKeyAndHash(ss, psk)
	re = message.ne
...
func SendMessage(session noisesession, message []byte) (noisesession, messagebuffer) {
	...
	hs = session.hs
	if session.mc == 0 {
		hs, messageBuffer, session.cs1, session.cs2 = writeMessageA(hs, message)
	}
...
func RecvMessage(session noisesession, message messagebuffer) (noisesession, []byte, bool) {
	...
	hs = session.hs
	if session.mc == 0 {
		hs, plaintext, valid, session.cs1, session.cs2 = readMessageA(hs, message)
	}
	session.mc = session.mc + 1
	session.hs = hs
