...
func writeMessageD(hs handshakestate, payload []byte) (handshakestate, messagebuffer, cipherstate, cipherstate) {
	ss, s, e, rs, re, psk, initiator := hs.ss, hs.s, hs.e, hs.rs, hs.re, hs.psk, hs.i
	ne, ns, ciphertext := emptyKey, []byte{}, []byte{}
...
func writeMessageE(hs handshakestate, payload []byte) (handshakestate, messagebuffer) {
	ss, s, e, rs, re, psk, initiator := hs.ss, hs.s, hs.e, hs.rs, hs.re, hs.psk, hs.i
	ne, ns, ciphertext := emptyKey, []byte{}, []byte{}
	ss, ciphertext = encryptAndHash(ss, payload)
	hs = handshakestate{ss, s, e, rs, re, psk, initiator}
...
func readMessageD(hs handshakestate, message messagebuffer) (handshakestate, []byte, bool, cipherstate, cipherstate) {
	ss, s, e, rs, re, psk, initiator := hs.ss, hs.s, hs.e, hs.rs, hs.re, hs.psk, hs.i
	valid1 := true
	ss = mixKey(ss, dh(s.sk, re))
	ss, plaintext, valid2 := decryptAndHash(ss, message.ciphertext)
...
func readMessageE(hs handshakestate, message messagebuffer) (handshakestate, []byte, bool) {
	ss, s, e, rs, re, psk, initiator := hs.ss, hs.s, hs.e, hs.rs, hs.re, hs.psk, hs.i
	valid1 := true
	ss, plaintext, valid2 := decryptAndHash(ss, message.ciphertext)
	if !valid2 {
...
func SendMessage(session noisesession, message []byte) (noisesession, messagebuffer) {
	...
	if session.mc == 3 {
		hs, messageBuffer, session.cs1, session.cs2 = writeMessageD(hs, message)
	}
	if session.mc > 3 {
		if hs.i {
		...
		hs, messageBuffer = writeMessageE(hs, message)
		if hs.i {
...
func RecvMessage(session noisesession, message messagebuffer) (noisesession, []byte, bool) {
	...
	if session.mc == 3 {
		hs, plaintext, valid, session.cs1, session.cs2 = readMessageD(hs, message)
	}
	if session.mc > 3 {
		if hs.i {
			hs.ss.cs = session.cs2
		...
		hs, plaintext, valid = readMessageE(hs, message)
		if hs.i {
			session.cs2 = hs.ss.cs
		} else {
			session.cs1 = hs.ss.cs
