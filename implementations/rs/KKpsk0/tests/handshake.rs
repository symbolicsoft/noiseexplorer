...
use noiseexplorer_kkpsk0::{noisesession::NoiseSession,
											...
											Keypair::from_private_key(initiator_ephemeral_private,)
										{
												...
												PrivateKey::from_str("4a6f686e2047616c74",)
											{
												if let Ok(responder_ephemeral_kp,) =
													...
													responder_session.set_ephemeral_keypair(
														responder_ephemeral_kp,
													);
													if let Ok(mA,) = Message::from_str(
														"4c756477696720766f6e204d69736573",
													) {
														if let Ok(tA,) = Message::from_str(
															"ca35def5ae56cec33dc2036731ab14896bc4c75dbb07a61f879f8e3afa4c794416088e45dd5bcdb9bee7037e09be96e5c9750d48aded34648f0663750995e4fa",
														) {
															if let Ok(messageA,) =
																initiator_session.send_message(mA,)
															{
																	...
																	.recv_message(messageA.clone(),)
																{
																	if let Ok(mB,) =
																		...
																		if let Ok(tB,) =
																			Message::from_str(
																				"95ebc60d2b1fa672c1f46a8aa265ef51bfe38e7ccb39ec5be34069f144808843fcf5c1d990871f224ffe090498a03bd50db64dcf448db09194f5a93e1aa73b",
																			) {
																			if let Ok(messageB,) =
																				responder_session
																					.send_message(
