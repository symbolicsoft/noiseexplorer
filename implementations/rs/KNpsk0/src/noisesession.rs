...
impl NoiseSession {
	...
	/// - `initiator`: `bool` variable. To be set as `true` when initiating a
	///   handshake with a remote party, or `false` otherwise.
	/// - `prologue`: `Message` object. Could optionally contain the name of the
	///   protocol to be used.
	/// - `s`: `Keypair` object. Contains local party's static keypair.
	/// - `rs`: `PublicKey` object. Contains the remote party's static public
	///   key.
	/// - `psk`: `Psk` object. Contains the pre-shared key.
	pub fn init_session(
		initiator: bool, prologue: Message, s: Keypair, rs: PublicKey, psk: Psk,
	) -> NoiseSession {
		if initiator {
			NoiseSession {
				hs:  HandshakeState::initialize_initiator(&prologue.as_bytes(), s, rs, psk,),
				mc:  0,
				i:   initiator,
				cs1: CipherState::new(),
				cs2: CipherState::new(),
			...
			NoiseSession {
				hs:  HandshakeState::initialize_responder(&prologue.as_bytes(), s, rs, psk,),
				mc:  0,
				i:   initiator,
				cs1: CipherState::new(),
				cs2: CipherState::new(),
	...
	/// Returns a `Ok(Message)` object containing the corresponding ciphertext
	/// upon successful encryption, and `Err(NoiseError)` otherwise
	///
	/// _Note that while `mc` <= 1 the ciphertext will be included as a payload
	/// for handshake messages and thus will not offer the same guarantees
	/// offered by post-handshake messages._
	...
	/// Returns a `Ok(Message)` object containing plaintext upon successful
	/// decryption, and `Err(NoiseError)` otherwise.
	///
	/// _Note that while `mc` <= 1 the ciphertext will be included as a payload
	/// for handshake messages and thus will not offer the same guarantees
	/// offered by post-handshake messages._
