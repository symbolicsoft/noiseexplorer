...
impl NoiseSession {
	...
	pub fn init_session(initiator: bool, prologue: Message, s: Keypair, rs: Option<PublicKey>, psk: Psk) -> NoiseSession {
		if initiator {
			NoiseSession{
				hs: HandshakeState::initialize_initiator(&prologue.as_bytes(), s, rs.unwrap_or(PublicKey::empty()), psk),
				mc: 0,
				i: initiator,
				cs1: CipherState::new(),
				cs2: CipherState::new(),
			...
			NoiseSession {
				hs: HandshakeState::initialize_responder(&prologue.as_bytes(), s, rs.unwrap_or(PublicKey::empty()), psk),
				mc: 0,
				i: initiator,
				cs1: CipherState::new(),
				cs2: CipherState::new(),
	...
	pub fn send_message(&mut self, message: Message, output: &mut [u8]) -> Result<usize, NoiseError> {
		let out: usize;
		if self.mc == 0 {
			let temp = self.hs.write_message_a(message.as_bytes(), output)?;
			self.h = temp.0;
			self.cs1 = temp.2;
	...
	pub fn recv_message(&mut self, message: Message, output: &mut [u8]) -> Result<usize, NoiseError> {
		let out: usize;
		if self.mc == 0 {
			let temp = self.hs.read_message_a(message.as_bytes(), output)?;
				self.h = temp.0;
				self.cs1 = temp.2;
				self.cs2 = CipherState::new();
				self.hs.clear();
