...
impl NoiseSession {
	...
	pub fn init_session(initiator: bool, prologue: Message, s: Keypair, rs: PublicKey, psk: Psk) -> NoiseSession {
		if initiator {
			NoiseSession{
	...
	pub fn recv_message(&mut self, input: &mut Vec<u8>) -> Option<Vec<u8>> {
		...
		if self.mc == 0 {
			if let Some(temp) = self.hs.read_message_a(input) {
				self.h = temp.0;
			...
			if self.i {
				if let Some(msg) = &self.cs1.read_message_regular(input) {
