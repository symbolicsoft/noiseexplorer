...
impl HandshakeState {
	...
	pub(crate) fn write_message_a(&mut self, input: &[u8]) -> Result<(Hash, Vec<u8>, CipherState, CipherState), NoiseError> {
	...
	pub(crate) fn read_message_a(&mut self, input: &[u8]) ->  Result<(Hash, Vec<u8>, CipherState, CipherState), NoiseError> {
		let rest = input;
		self.ss.mix_key_and_hash(&self.psk.as_bytes());
