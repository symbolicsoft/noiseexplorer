...
impl HandshakeState {
	...
	pub(crate) fn initialize_initiator(prologue: &[u8], s: Keypair, rs: PublicKey, psk: Psk) -> HandshakeState {
		let protocol_name = b"Noise_Kpsk0_25519_ChaChaPoly_BLAKE2s";
	...
	pub(crate) fn write_message_a(&mut self, input: &[u8], output: &mut [u8]) -> Result<(Hash, usize, CipherState, CipherState), NoiseError> {
	...
	pub(crate) fn read_message_a(&mut self, input: &[u8], output: &mut [u8]) ->  Result<(Hash, usize, CipherState, CipherState), NoiseError> {
		self.ss.mix_key_and_hash(&self.psk.as_bytes());
