...
impl HandshakeState {
	...
	pub(crate) fn write_message_a(&mut self, payload: &[u8]) -> ((Hash, Vec<u8>, CipherState, CipherState)) {
	...
	pub(crate) fn read_message_a(&mut self, input: &mut Vec<u8>) -> ( Option<(Hash, Vec<u8>, CipherState, CipherState)>) {
		let rest = input;
		self.ss.mix_key_and_hash(&self.psk.as_bytes());
