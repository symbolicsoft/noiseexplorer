...
fn test() {
	...
	let initStaticA: X1K::Keypair = X1K::Keypair::new_k(decode_str_32("e61ef9919cde45dd5f82166404bd08e38bceb5dfdfded0a34c8df7ed542214d1"));
	let respStatic: X1K::Keypair = X1K::Keypair::new_k(decode_str_32("4a3acbfdb163dec651dfa3194dece676d437029c62a408b4c5ea9114246e4893"));
	let mut initiatorSession: X1K::NoiseSession =
	X1K::NoiseSession::InitSession(true, &prologue, initStaticA, respStatic.pk.0);
	let mut responderSession: X1K::NoiseSession =
	X1K::NoiseSession::InitSession(false, &prologue, respStatic, X1K::EMPTY_KEY);
	initiatorSession.set_ephemeral_keypair(X1K::Keypair::new_k(decode_str_32(
		"893e28b9dc6ca8d611ab664754b8ceb7bac5117349a4439a6b0569da977c464a"
	)));
	responderSession.set_ephemeral_keypair(X1K::Keypair::new_k(decode_str_32(
		"bbdb4cdbd309f1a1f2e1456967fe288cadd6f712d65dc7b7793d5e63da6b375b"
	)));
	let payloadA = decode_str("4c756477696720766f6e204d69736573");
	let mut messageA: X1K::MessageBuffer = initiatorSession.SendMessage(&payloadA);
	...
	let mut messageB: X1K::MessageBuffer = responderSession.SendMessage(&payloadB);
	...
	let mut messageC: X1K::MessageBuffer = initiatorSession.SendMessage(&payloadC);
	...
	let mut messageD: X1K::MessageBuffer = responderSession.SendMessage(&payloadD);
	...
	let mut messageE: X1K::MessageBuffer = initiatorSession.SendMessage(&payloadE);
	...
	let mut messageF: X1K::MessageBuffer = responderSession.SendMessage(&payloadF);
