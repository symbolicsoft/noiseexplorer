...
impl NoiseSession {
	...
	pub fn send_message(&mut self, message: Message, output: &mut [u8]) -> Result<usize, NoiseError> {
		...
		if self.mc == 0 {
			...
			out = self.hs.write_message_c(message.as_bytes(), output)?;
		}
		else if self.mc == 3 {
			let temp = self.hs.write_message_d(message.as_bytes(), output)?;
			self.h = temp.0;
			self.cs1 = temp.2;
			self.cs2 = temp.3;
			self.hs.clear();
	...
	pub fn recv_message(&mut self, message: Message, output: &mut [u8]) -> Result<usize, NoiseError> {
		...
		if self.mc == 0 {
			...
			out = self.hs.read_message_c(message.as_bytes(), output)?;
		}
		else if self.mc == 3 {
			let temp = self.hs.read_message_d(message.as_bytes(), output)?;
				self.h = temp.0;
				self.cs1 = temp.2;
				self.cs2 = temp.3;
				self.hs.clear();
