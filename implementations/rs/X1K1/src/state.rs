...
impl HandshakeState {
	...
	pub(crate) fn initialize_initiator(prologue: &[u8], s: Keypair, rs: PublicKey, psk: Psk) -> HandshakeState {
		let protocol_name = b"Noise_X1K1_25519_ChaChaPoly_BLAKE2s";
	...
	pub(crate) fn write_message_c(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, NoiseError> {
	...
	pub(crate) fn write_message_d(&mut self, input: &[u8], output: &mut [u8]) -> Result<(Hash, usize, CipherState, CipherState), NoiseError> {
	...
	pub(crate) fn read_message_c(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, NoiseError> {
	...
	pub(crate) fn read_message_d(&mut self, input: &[u8], output: &mut [u8]) ->  Result<(Hash, usize, CipherState, CipherState), NoiseError> {
		self.ss.mix_key(&self.s.dh(&self.re.as_bytes()));
		let len = self.ss.decrypt_and_hash(input, output)?;
		let h: Hash = Hash::from_bytes(from_slice_hashlen(&self.ss.h.as_bytes()));
		let (cs1, cs2) = self.ss.split();
