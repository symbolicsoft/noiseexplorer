...
impl HandshakeState {
	...
	pub(crate) fn write_message_b(&mut self, payload: &[u8]) -> (Vec<u8>) {
		let mut output: Vec<u8> = Vec::new();
		if self.e.is_empty() {
			self.e = Keypair::new();
		}
	...
	pub(crate) fn write_message_c(&mut self, payload: &[u8]) -> (Vec<u8>) {
		let mut output: Vec<u8> = Vec::new();
	...
	pub(crate) fn write_message_d(&mut self, payload: &[u8]) -> ((Hash, Vec<u8>, CipherState, CipherState)) {
		let mut output: Vec<u8> = Vec::new();
	...
	pub(crate) fn read_message_b(&mut self, input: &mut Vec<u8>) -> (Option<Vec<u8>>) {
		let rest = input;
		let (vre, rest) = rest.split_at(32);
		self.re = PublicKey::from_bytes(from_slice_hashlen(&vre.to_owned()[..]));
		self.ss.mix_hash(&self.re.as_bytes()[..DHLEN]);
	...
	pub(crate) fn read_message_d(&mut self, input: &mut Vec<u8>) -> ( Option<(Hash, Vec<u8>, CipherState, CipherState)>) {
		let rest = input;
		self.ss.mix_key(&self.s.dh(&self.re.as_bytes()));
		if let Some(plaintext) = self.ss.decrypt_and_hash(&rest) {
			let h: Hash = Hash::from_bytes(from_slice_hashlen(&self.ss.h.as_bytes()));
