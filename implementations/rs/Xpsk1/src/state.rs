...
impl HandshakeState {
	...
	pub(crate) fn write_message_a(&mut self, in_out: &mut [u8]) -> Result<(Hash, CipherState, CipherState), NoiseError> {
		if in_out.len() < DHLEN {
	...
	pub(crate) fn read_message_a(&mut self, in_out: &mut [u8]) ->  Result<(Hash, CipherState, CipherState), NoiseError> {
