...
fn InitializeInitiator(prologue: &[u8], s: Keypair, rs: [u8; DHLEN], psk: [u8; PSK_LENGTH]) -> HandshakeState {
	let protocol_name = b"Noise_KNpsk0_25519_ChaChaPoly_BLAKE2s";
...
fn main() {
	...
	let respStatic: Keypair = Keypair::new_k(decode_str_32("EMPTY_KEY"));
	let temp_psk1: [u8; 32] =
	decode_str_32("54686973206973206d7920417573747269616e20706572737065637469766521")
	let temp_psk2: [u8; 32] =
	decode_str_32("54686973206973206d7920417573747269616e20706572737065637469766521")
	let mut initiatorSession: NoiseSession =
	NoiseSession::InitSession(true, &prologue, initStatic, temp_psk1);
	let mut responderSession: NoiseSession =
	NoiseSession::InitSession(false, &prologue, respStatic, initStatic.pk.0, temp_psk2);
