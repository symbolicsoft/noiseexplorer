...
fn InitializeInitiator(prologue: &[u8], s: Keypair, rs: [u8; DHLEN], psk: [u8; PSK_LENGTH]) -> HandshakeState {
	let protocol_name = b"Noise_X1K_25519_ChaChaPoly_BLAKE2s";
...
fn ReadMessageC(&mut self, message: &mut MessageBuffer) -> (Option<Vec<u8>>) {
...
fn ReadMessageD(&mut self, message: &mut MessageBuffer) -> (Option<([u8; 32], Vec<u8>, CipherState, CipherState)>) {
	self.ss.MixKey(&DH(&self.s, &self.re));
	if let Some(plaintext) = self.ss.DecryptAndHash(&message.ciphertext) {
		let (cs1, cs2) = self.ss.Split();
...
impl NoiseSession {
	...
	pub fn SendMessage(&mut self, message: &[u8]) -> MessageBuffer {
		...
		&& self.hs.ss.cs.n < MAX_NONCE && message.len() < 65535 {
			...
			if self.mc == 2 {
				buffer = self.hs.WriteMessageC(message);
			}
			if self.mc == 3 {
				let temp = self.hs.WriteMessageD(message);
				self.h = temp.0;
				buffer = temp.1;
				self.cs1 = temp.2;
				self.cs2 = temp.3;
	...
	pub fn RecvMessage(&mut self, message: &mut MessageBuffer) -> Option<Vec<u8>> {
		...
		&& self.hs.ss.cs.n < MAX_NONCE && message.ciphertext.len() < 65535 {
			...
			if self.mc == 2 {
				plaintext = self.hs.ReadMessageC(message);
			}
			if self.mc == 3 {
...
fn X1K() {
	...
	NoiseSession::InitSession(true, &prologue, initStatic, respStatic.pk.0, EMPTY_KEY
