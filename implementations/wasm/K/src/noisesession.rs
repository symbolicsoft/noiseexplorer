...
impl NoiseSession {
	...
	pub fn init_session(initiator: bool, prologue: &[u8], s: Keypair, rs: Option<PublicKey>) -> NoiseSession {
		if initiator {
			...
			NoiseSession {
				hs: HandshakeState::initialize_responder(prologue, s, rs.unwrap_or(PublicKey::empty()), Psk::default()),
				mc: 0,
				i: initiator,
				cs1: CipherState::new(),
				cs2: CipherState::new(),
	...
	pub fn send_message(&mut self, in_out: &mut [u8]) -> Result<(), NoiseError> {
		...
		if self.mc == 0 {
			let temp = self.hs.write_message_a(in_out)?;
			self.h = temp.0;
	...
	pub fn recv_message(&mut self, in_out: &mut [u8]) -> Result<(), NoiseError> {
		...
		if self.mc == 0 {
			let temp = self.hs.read_message_a(in_out)?;
				self.h = temp.0;
			self.is_transport = true;
				self.cs1 = temp.1;
				self.cs2 = CipherState::new();
