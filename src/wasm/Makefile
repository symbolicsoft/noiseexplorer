...
bundler:
	...
	@wasm-pack build --target bundler --release --scope noiseexplorer_$NOISE2WASM_N$_wasm
	@/bin/echo " OK."

nodejs:
	@/bin/echo  "[NoiseExplorer] Compiling Node friendly WASM Module..."
	@wasm-pack build --target nodejs --release --scope noiseexplorer_$NOISE2WASM_N$_wasm
	@/bin/echo " OK."

no-modules:
	@/bin/echo  "[NoiseExplorer] Compiling WASM Module..."
	@wasm-pack build --target no-modules --release --scope noiseexplorer_$NOISE2WASM_N$_wasm
	@/bin/echo " OK."

web:
	@/bin/echo  "[NoiseExplorer] Compiling WASM Module..."
	@wasm-pack build --target web --release --scope noiseexplorer_$NOISE2WASM_N$_wasm
	@/bin/echo " OK."
